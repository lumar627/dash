...
bot = telebot.TeleBot('8190247212:AAH6kzB0MA-U8Y-C0sszJjAvtXgyTjKr9vI')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "You have not purchased yet purchase now from:- @VILAXLORD."
...
def clear_users_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "ꜰʀᴇᴇ ᴋᴇ ᴅʜᴀʀᴍ ꜱʜᴀʟᴀ ʜᴀɪ ᴋʏᴀ ᴊᴏ ᴍᴜ ᴜᴛᴛʜᴀ ᴋᴀɪ ᴋʜɪ ʙʜɪ ɢᴜꜱ ʀʜᴀɪ ʜᴏ ʙᴜʏ ᴋʀᴏ ꜰʀᴇᴇ ᴍᴀɪ ᴋᴜᴄʜ ɴʜɪ ᴍɪʟᴛᴀ ʙᴜʏ:- @VILAXLORD 🙇."
    bot.reply_to(message, response)
...
def start_attack_reply(message, target, port, time):
    ...
    response = f" 🚀 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃 🚀\n\n🌀 𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n🎯 𝐏𝐨𝐫𝐭: {port}\n🕛 𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n🛠️ 𝐌𝐞𝐭𝐡𝐨𝐝: BGMI KI CHIN TAPAK DAM DAM 😝"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 600:
                ...
                response = f"💦SERVER AAGYA CHUTIYA REJOIN KAR ☠️. 🌀 Target: {target} 🎯 Port: {port} 🕛 Time: {time}"
                bot.reply_to(message, response)  # Notify the user that the attack is finished
        else:
            response = "✅ Usage :- /bgmi <target> <port> <time>"  # Updated command syntax
    else:
        response = ("🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /bgmi command. DM TO BUY ACCESS:- @VILAXLORD")
...
Buy From :- @VILAXLORD
Official Channel :- https://t.me/+zwVMuLrc_XhiZTI1
'''
...
3. MAKE SURE YOU JOINED https://t.me/+zwVMuLrc_XhiZTI1 OTHERWISE NOT WORK
